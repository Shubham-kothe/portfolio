...
const cardVariants = {
  ...
  visible: { y: 0, opacity: 1, scale: 1, transition: { duration: 0.6, ease: [0.25,0.1,0.25,1] as const } }
};

export const Education = () => {
  const { elementRef, isIntersecting } = useIntersectionObserver({ threshold: 0.25 });

  return (
    <section id="education" ref={elementRef} className="section-padding relative">
      <div className="section-container">
        ...
        >
          <motion.div variants={cardVariants} className="text-center max-w-3xl mx-auto">
            ...
            <p className="mt-6 text-lg prose-muted">Academic journey that shaped my engineering and AI foundations.</p>
          </motion.div>

          <div className="grid md:grid-cols-2 gap-10">
            {education.map((item) => (
              <motion.div key={item.id} variants={cardVariants} whileHover={{ y: -6 }} className="relative group">
                ...
                <div className="relative h-full rounded-2xl p-1 bg-gradient-to-br from-white/80 to-white/40 dark:from-slate-800/70 dark:to-slate-800/30 backdrop-blur-xl border border-white/40 dark:border-white/10 shadow-[0_8px_30px_-8px_rgba(99,102,241,0.35)]">
                  <div className="rounded-xl h-full bg-white/70 dark:bg-slate-900/70 p-6 flex flex-col gap-5">
                    <div className="flex items-center gap-4">
                      <div className={`relative w-16 h-16 rounded-xl bg-gradient-to-br ${item.accent} flex items-center justify-center text-white shadow-lg overflow-hidden`}>                        
                        {item.logo === 'bits' ? (
                      ...
                      <div>
                        <h3 className="text-xl font-bold tracking-tight text-slate-900 dark:text-slate-100">{item.institute}</h3>
                        <p className="text-sm font-medium text-indigo-600 dark:text-indigo-300 mt-1">{item.course}</p>
                      </div>
                    </div>
